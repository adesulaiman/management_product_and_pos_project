...
$qFieldSub = $adeQ->select($adeQ->prepare(
  "select * from core_fields where id_form=%d and active is true and type_field='sub' order by id", $f
));
...
$qFieldimage = $adeQ->select($adeQ->prepare(
  "select * from core_fields where id_form=%d and active is true and type_input in ('image', 'file') order by id", $f
));
...
?>
    ...
    <section class="content-header">
      <h1>
        <button onclick="HtmlLoad('./lib/base/<?php echo $script ?>?f=<?php echo $fParent ?>','');" class='btn btn-danger'><i class="fa fa-angle-left"></i> Back</button> <?php echo $formDesc ?>
      </h1>
      <ol class="breadcrumb">
...
$('.filterAdvCheck').click(function(){
    ...
    var table = $("#<?php echo $formName ?>").DataTable({
      ...
      "serverSide": true,
      "ajax": {
        "url" : "./lib/base/load_data_with_date_sub_so.php?w=<?php echo $qFieldSub[0]['name_field'].'='.$sub;?>&t=<?php echo $formView ?>&f=<?php echo $f?>",
        "data" : function(data){
