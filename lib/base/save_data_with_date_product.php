	...
	if(isset($_POST['f']) and isset($_POST['formType']))
  	{
		...
		switch ($formType) {
			case 'add':
				...
				if($stt)
				{
					$q = "insert into $formCode (".implode(",", $fieldNm).", barcode) values (".implode(",", $ins).", '".uniqid()."')";
					$ins = $adeQ->query($q);
					$craeteBarcode = $adeQ->query("
						update data_product set barcode = right(concat('000000', id), 6)
						where
						length(barcode) = 13
					");

					$msg = 'Data Berhasil Di Simpan';
					if(!$ins or !$craeteBarcode)
					{
			...
			case 'delete' :
				...
				if($stt)
				{
					$q = "update $formCode set is_delete = 1 where $where";
					$del = $adeQ->query($q);
