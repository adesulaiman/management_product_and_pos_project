	...
	if(isset($_GET['f']) and isset($_GET['t']))
  	{
		...
		$qField = $adeQ->select($adeQ->prepare(
		    "select * from information_schema.columns where table_name=%s and table_schema=%s order by ordinal_position", $t, $dbName
		));
		...
		if(empty($query))
		{
			$where = " where id_user = '$_SESSION[userid]'";
		}else{
			$where = "where id_user = '$_SESSION[userid]' and $query";
		}
