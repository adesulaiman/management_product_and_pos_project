	...
	if(isset($_GET['f']) and isset($_GET['t']))
  	{
		...
		if($iscommit){
			...
			$t = "vw_data_stock_weight";

			
		}else{
			$t = "data_stock_weight_detail_hist";
		}
		

		$qField = $adeQ->select($adeQ->prepare(
		    "select * from information_schema.columns where table_name=%s order by ordinal_position", 'vw_data_stock_weight'
		));
