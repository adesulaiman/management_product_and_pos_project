...
$cekdata = $adeQ->select("select * from data_cost_daily where id= $sub");
$commit = $cekdata[0]['status'];
...
<script>
    ...
    var table = $("#<?php echo $formName ?>").DataTable({
        ...
        buttons: [
            ...
            {
                text: '<i class="fa fa-check"></i> Commit',
                className: "btn btn-success btn-lg",
                action: function(e, dt, node, config) {
                    swal({
                            title: "Commit Cost ?",
                            text: "Once commit Cost, you will not be able to back this Cost !",
                            icon: "warning",
                            buttons: true,
                            dangerMode: true,
                        })
                        .then((willtransaction) => {
                            if (willtransaction) {
                                ...
                                $.ajax({
                                    method: "POST",
                                    url: "./lib/base/save_data_with_date_cost.php",
                                    data: formData,
                                    dataType: 'json',
